...
echo -e "${YELLOW}Starting NPM HEAVY Stress Test${NC}"
...
setup_env() {
    ...
    curl -s -X POST "$API_URL/repositories" -H "Content-Type: application/json" -H "$AUTH_HEADER" -d "{\"name\": \"$REPO_NAME\", \"type\": \"hosted\", \"manager\": \"npm\", \"config\": {}}" > /dev/null
}
...
run_worker() {
    ...
    local tmpdir="/tmp/npm-stress-$worker_id"
    mkdir -p "$tmpdir"

    for ((i=1; i<=iterations; i++)); do
        local pkg_name="heavy-pkg-w${worker_id}-i${i}"
        local version="1.0.${i}"
        local tarball="${pkg_name}-${version}.tgz"
        local tarball_path="$tmpdir/$tarball"
        
        # Create package with large binary data (1-5MB)
        local pkg_dir="$tmpdir/package"
        mkdir -p "$pkg_dir"
        
        # Generate large data file
        local data_size=$((1 * 1024 * 1024 + RANDOM % (4 * 1024 * 1024)))
        dd if=/dev/urandom of="$pkg_dir/data.bin" bs=1024 count=$((data_size / 1024)) 2>/dev/null
        ...
        cat > "$pkg_dir/package.json" <<EOF
{
  "name": "$pkg_name",
  "version": "$version",
  "description": "Heavy stress test package with binary data"
}
EOF
        
        # Create tarball
        tar -czf "$tarball_path" -C "$tmpdir" package 2>/dev/null
        
        # Publish
        HTTP_CODE=$(curl -s -w "%{http_code}" -X PUT "$REPOS_URL/$REPO_NAME/$pkg_name/-/$tarball" \
            -H "$AUTH_HEADER" -H "Content-Type: application/octet-stream" \
            --data-binary "@$tarball_path" -o /dev/null)
        
        # Cleanup temp files
        rm -rf "$pkg_dir" "$tarball_path"

        if [ "$HTTP_CODE" == "200" ] || [ "$HTTP_CODE" == "201" ]; then
            # Verify metadata (lightweight check)
    ...
    echo "$worker_id $success_count $fail_count $duration" > "/tmp/stress/npm-heavy-result-$worker_id.txt"
}

rm -rf /tmp/stress/npm-heavy
mkdir -p /tmp/stress
...
total_fail=0
for ((k=1; k<=CONCURRENCY; k++)); do
    if [ -f "/tmp/stress/npm-heavy-result-$k.txt" ]; then
        read w s f d < "/tmp/stress/npm-heavy-result-$k.txt"
        total_success=$((total_success + s))
        total_fail=$((total_fail + f))
    fi
done

echo "--- NPM HEAVY Results ---"
