...
export async function handleMavenGroupUpload(
  context: PluginContext,
  repo: Repository,
  pkg: any,
...
export async function handleMavenGroupPut(
  context: PluginContext,
  repo: Repository,
  repoPath: string,
  req: any,
  ...
  const delegateRequest = { body: buffer };

  if (writePolicy === 'first') {
    const hosted = await getHostedMembers(context, members);
    for (const member of hosted) {
      try {
        const result = await handlePut(member, repoPath, delegateRequest);
        if (result.ok) return result;
      } catch (error) {
        console.warn(
          `[Maven] Group write failed for member ${member.id}: ${String(error)}`,
        );
      }
    }
    return { ok: false, message: 'No writable member found' };
  ...
  if (writePolicy === 'preferred' || writePolicy === 'broadcast') {
    ...
    return await handlePut(member, repoPath, delegateRequest);
  }
  ...
  if (writePolicy === 'mirror') {
    ...
    const results = await Promise.all(
      hosted.map((member) => handlePut(member, repoPath, delegateRequest)),
    );
    return (
      results.find((result) => result.ok) || {
        ok: false,
