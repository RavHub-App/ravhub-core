...
export function resolveSnapshotVersion(metadata: any, extension: string, classifier?: string): string | null {
    const versioning = metadata?.metadata?.versioning;
    if (!versioning) return null;

    // Strategy 1: Use snapshotVersions list (preferred)
    if (versioning.snapshotVersions && versioning.snapshotVersions.snapshotVersion) {
    ...
    if (!classifier && versioning.snapshot && versioning.snapshot.timestamp && versioning.snapshot.buildNumber) {
...
export function parseFilename(filename: string, version: string, artifactId: string) {
