...
export class UnifiedPermissionGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (
      this.isReadinessCheck(req) ||
      this.isDockerTokenRequest(req) ||
      this.isDockerRegistryRequest(req)
    ) {
    ...
    if (!req.user || !req.user.id) {
      this.logger.debug(
        `UnifiedPermissionGuard: no authenticated user found for ${url}`,
      );
  ...
  private isDockerTokenRequest(req: any): boolean {
    try {
      const url = req.originalUrl || req.url || '';
      return url && typeof url === 'string' && /\/v2\/token(\/|\?|$)/.test(url);
    } catch {
      return false;
    }
  }
  ...
  private isDockerRegistryRequest(req: any): boolean {
    try {
      ...
      return url && typeof url === 'string' && /\/v2\//.test(url);
    } catch {
      return false;
    }
  }
