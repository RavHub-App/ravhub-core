...
describe('PyPIPlugin Storage', () => {
  ...
  describe('download', () => {
    it('should encode repository names in hosted simple index links', async () => {
      const encodedRepo = { id: 'r1', name: 'pypi repo#beta', type: 'hosted' };
      context.storage.list.mockResolvedValue([
        'pypi/r1/flask/2.0.0/flask-2.0.0.tar.gz',
      ]);

      const result = await storageMethods.download(encodedRepo, 'simple/flask');

      expect(result.ok).toBe(true);
      expect(String(result.data)).toContain(
        'http://localhost:3000/repository/pypi%20repo%23beta/flask/2.0.0/flask-2.0.0.tar.gz',
      );
    });

    it('should warn and return fallback simple index when hosted listing fails', async () => {
      const warnSpy = jest
        .spyOn(console, 'warn')
        .mockImplementation(() => undefined);
      context.storage.list.mockRejectedValue(new Error('list-fail'));

      const result = await storageMethods.download(repo, 'simple/flask');

      expect(result.ok).toBe(true);
      expect(String(result.data)).toContain('<h1>Links for flask</h1>');
      expect(warnSpy).toHaveBeenCalledWith(
        '[PyPIPlugin] Failed to list hosted simple index files for flask: Error: list-fail',
      );
      warnSpy.mockRestore();
    });
    ...
    it('should list and pick preferred format if exact match fails', async () => {
      ...
      context.storage.get.mockImplementation((key: string) => {
        if (key.endsWith('.whl'))
          return Promise.resolve(Buffer.from('whl content'));
    ...
    it('should continue group download when one member lookup fails', async () => {
      ...
      expect(result.ok).toBe(true);
      expect(warnSpy).toHaveBeenCalledWith(
        '[PyPIPlugin] Failed to resolve repository broken: Error: member-fail',
      );
      warnSpy.mockRestore();
    });
  ...
  describe('download (proxy)', () => {
    ...
    it('should ignore indexing errors in proxy download', async () => {
      ...
      expect(result.ok).toBe(true);
      expect(warnSpy).toHaveBeenCalledWith(
        '[PyPIPlugin] Failed to index proxied artifact flask:2.0.0: Error: index fail',
      );
      warnSpy.mockRestore();
    });
  });
