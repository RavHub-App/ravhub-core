  ...
  describe('🐳 Docker Hosted Repositories', () => {
    ...
    it('should upload a blob to the registry', async () => {
      ...
      const initRes = await axios.post(
        `http://localhost:${registryPort}/v2/${imageName}/blobs/uploads/`,
        {},
        {
      ...
      const fullUploadUrl = uploadUrl.startsWith('http')
        ? uploadUrl
        : `http://localhost:${registryPort}${uploadUrl}`;

      const finalizeRes = await axios.put(
        `${fullUploadUrl}?digest=${digest}`,
        blobContent,
        {
          headers: {
    ...
    it('should push a manifest', async () => {
      ...
      const res = await axios.put(
        `http://localhost:${registryPort}/v2/${imageName}/manifests/${tag}`,
        manifest,
        {
          headers: {
            Authorization: `Bearer ${dockerToken}`,
    ...
    it('should list tags for the image', async () => {
      const res = await axios.get(
        `http://localhost:${registryPort}/v2/${imageName}/tags/list`,
        {
          headers: { Authorization: `Bearer ${dockerToken}` },
        },
      );
    ...
    it('should find the image in the packages list', async () => {
      ...
      expect(found.latestVersion).toBe(tag);
    });

    it('should return package details for image names with slash', async () => {
      const res = await request(context.app.getHttpServer())
        .get(`/api/repository/${repoId}/packages/${encodeURIComponent(imageName)}`)
        .set('Authorization', `Bearer ${authToken}`)
        .expect(200);
      ...
      expect(res.body.artifacts).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            version: tag,
            installCommand: expect.stringContaining(`docker pull localhost:${registryPort}/${imageName}:${tag}`),
          }),
        ]),
      );
    });
