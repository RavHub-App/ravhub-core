...
describe('NuGet Repository E2E', () => {
  ...
  describe('📦 NuGet Hosted Repositories (V3)', () => {
    ...
    it('should upload a nuget package (.nupkg) via PUT', async () => {
      ...
      expect(res.body.id.toLowerCase()).toBe(
        `${pkgName.toLowerCase()}:${pkgVersion}`,
      );
    });
    ...
    it('should download the V3 service index', async () => {
      ...
      const flatContainer = res.body.resources.find(
        (r: any) => r['@type'] === 'PackageBaseAddress/3.0.0',
      );
      expect(flatContainer).toBeDefined();
      expect(flatContainer['@id']).toContain(
        `/repository/${repoName}/v3/flatcontainer/`,
      );
    });
    ...
    it('should download the package using flatcontainer path', async () => {
      ...
      const res = await request(context.app.getHttpServer())
        .get(
          `/api/repository/${repoName}/v3/flatcontainer/${pkgName.toLowerCase()}/${pkgVersion}/${fileName}`,
        )
        .expect(200);
    ...
    it('should list packages and find the uploaded one', async () => {
      ...
      const found = res.body.packages.find(
        (p: any) => p.name.toLowerCase() === pkgName.toLowerCase(),
      );
    ...
    it('should enforce no-redeploy policy if configured', async () => {
      ...
      expect(res.body.message.toLowerCase()).toContain(
        `redeployment of ${pkgName.toLowerCase()}:${pkgVersion} is not allowed`,
      );
    });
  });
