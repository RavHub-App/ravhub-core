...
describe('PyPI Advanced Scenarios (Concurrency & PEP 503)', () => {
  ...
  it('should return PEP 503 HTML for simple index', async () => {
    ...
    await request(context.app.getHttpServer())
      .put(
        `/api/repository/${repoName}/${pkgName}/${version}/${pkgName}-${version}.tar.gz`,
      )
      .set('Authorization', `Bearer ${authToken}`)
      .send('dummy-content')
      .expect(200);
    ...
    expect(res.text).toContain(
      `href="http://localhost:3000/repository/${repoName}/${pkgName}/${version}/${pkgName}-${version}.tar.gz"`,
    );
  });
  ...
  it('should handle concurrent downloads of the same artifact successfully', async () => {
    ...
    const downloads: Array<Promise<import('supertest').Response>> = [];
