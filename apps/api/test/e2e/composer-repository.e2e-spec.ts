...
describe('Composer Repository E2E', () => {
  ...
  describe('🎼 Composer Hosted Repositories', () => {
    ...
    it('should download the packages.json metadata', async () => {
      ...
      expect(
        res.body.packages[`${pkgVendor}/${pkgPackage}`][pkgVersion],
      ).toBeDefined();
      ...
      expect(pkgData.dist.url).toContain(
        `${pkgVendor}/${pkgPackage}/${pkgVersion}.zip`,
      );
    });
    ...
    it('should download the uploaded ZIP', async () => {
      ...
      const content = Buffer.isBuffer(res.body)
        ? res.body.toString()
        : res.text;
    ...
    it('should list packages and find the uploaded one', async () => {
      ...
      const found = res.body.packages.find(
        (p: any) => p.name === `${pkgVendor}/${pkgPackage}`,
      );
  ...
  describe('🌐 Composer Proxy Repositories', () => {
    ...
    it('should download packages.json from proxy (not implemented fully in test, but check status)', async () => {
      ...
      const res = await request(context.app.getHttpServer()).get(
        `/api/repository/${repoName}/packages.json`,
      );
